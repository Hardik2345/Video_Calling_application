...
export const getLocalPreview = () => {
  navigator.mediaDevices
    ...
    .catch((err) => {
      console.log("error occoured when trying to get access to your camera");
      console.log(err);
    });
};
...
const createPeerConnection = () => {
  ...
  peerConnection.onconnectionstatechange = (event) => {
    if (peerConnection.connectionState === "connected") {
      console.log("succesfully connected with the other peer");
    }
  };
...
export const handlePreOfferAnswer = (data) => {
  ...
  if (preOfferAnswer === constants.preOfferAnswer.CALL_REJECTED) {
    setIncomingCallsAvailable();
    ui.showInfoDialog(preOfferAnswer);
    //show dialog that the call is rejected
  }
...
export const handleWebRTCCandidate = async (data) => {
  try {
    ...
    console.log("error occoured when trying to add recieved ice candidate");
  }
};
...
export const switchBetweenCameraAndScreenSharing = async (
  ...
  if (screenSharingActive) {
    try {
      ...
      console.error("Error occurred when switching back to the camera", err);
      alert("Camera access denied. Please check browser settings.");
    }
  } else {
    try {
      ...
      // Replace camera track with screen-sharing track
      const videoSender = peerConnection
        .getSenders()
        .find((sender) => sender.track?.kind === "video");
      if (videoSender) {
